...
begin
  ...
  for i := 0 to High (XTitles) do begin
    ExcelApp.Cells[1, i + 2].Font.Bold := True;
    ExcelApp.Cells[1, i + 2].HorizontalAlignment := 3;
    ExcelApp.Cells[1, i + 2] := WideString(UTF8ToSys(XTitles[i].Caption));
  end;

  for i := 0 to High(aData) do begin
    ExcelApp.Cells[i + 2, 1].Font.Bold := True;
    ExcelApp.Cells[i + 2, 1].VerticalAlignment := 2;
    ExcelApp.Cells[i + 2, 1].HorizontalAlignment := 3;
    ExcelApp.Cells[i + 2, 1].ColumnWidth := 20;
    ExcelApp.Cells[i + 2, 1] :=  WideString(UTF8ToSys(YTitles[i].Caption));

    for j := 0 to High(aData[i]) do begin

      ExcelApp.Cells[i + 2, j + 2].VerticalAlignment := 1;
      ExcelApp.Cells[i + 2, j + 2].ColumnWidth := 50;
      ExcelApp.Cells[i + 2, j + 2].WrapText := True;
      ExcelApp.Cells[i + 2, j + 2].Borders.LineStyle := 1;
      ...
      ExcelApp.Cells[i + 2, j + 2] :=  WideString(UTF8ToSys(s));
    end;
  end;
...
begin
  ...
  Writeln(fout, '<tr valign = "top"><td></td>');
  for i := 0 to High (XTitles) do begin
    Writeln(fout, '<td class = "h">' + XTitles[i].Caption + '</td>');
  end;
  Writeln(fout, '</tr>');

  for i := 0 to High(aData) do begin
    Writeln(fout, '<tr valign = "top"><td class = "h">');
    ...
    for j := 0 to High(aData[i]) do begin
      ...
      for k := 0 to High(aData[i][j]) do begin
        ...
        for l := 0 to High(aData[i][j][k].Item) do begin
          if not ScheduleForm.VisibleColumn[l] then Continue;
